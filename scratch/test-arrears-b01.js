...
// Copy necessary calendar/date helper functions from src/lib/actions/qr.ts and getArrearMonths
const toEthiopian = (date) => {
  const k = new Kenat(date);
  return { year: k.year, month: k.month, day: k.day };
};

const getEthiopianMonthEnd = (date) => {
  const k = new Kenat(date);
  // Get Gregorian date of last day of this Ethiopian month
...
function getArrearMonths(leaseStart, payments) {
  ...
  while (iterations < 60) {
    ...
    if (!coveredMonthKeys.has(key)) {
      ...
      arrears.push(new Date(Date.UTC(greg.getFullYear(), greg.getMonth(), greg.getDate(), 12, 0, 0)));
    }
